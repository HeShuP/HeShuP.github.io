...
从record中，解析出has_image、apply_image：
  ...
  1. 根据pg_control记录的checkpoint信息，结合xlog日志，获取redo点；
  1. 循环从redo点读取wal记录；
  1. 解析wal记录中，获取DecodedBkpBlock结构；
  1. 根据DecodedBkpBlock中的has_image、apply_image等参数，决定是否从WAL中读取image，恢复数据，使PostgreSQL重新达到数据一致性。
